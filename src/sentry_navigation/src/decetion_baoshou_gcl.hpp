	...
	case 0:
		navigate_with_yaw_to(location1, 1, [this](bool success) {
			if (success) {
				...
				wait_for_(25,[this](){multiple_palce_nav();});
			} else {
...
void DecisionNode::recover_time_out_detect_handle()
{
	...
	{
		if((ros::Time::now()-recover_time).toSec()>40)
		{	
			recover_time_out=true;
			home_xxunluo();
		}
	}
}
...
void DecisionNode::xunluo_waiting()
{
	...
	if((ros::Time::now()-xunluo_time).toSec()>20)
	{
...
void DecisionNode::start() {
	...
	if(state!=State::WAIT_FOR_START)
	{
		...
		if(cmp_info.our_sentry_hp<=100&&state!=State::NAV_RECOVER_HP&&state!=State::RECOVERING_HP&&recover_time_out==false)
		{
			to_recover_place();
		}

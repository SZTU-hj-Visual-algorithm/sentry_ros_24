...
std::vector<Armor> NeuralArmorDetector::neuralArmorGrade(const std::vector<Detection>& candidateArmors) {
    ...
    for (const auto &candidateArmor: candidateArmors) {
        Armor armor;
        // 校验中点
        double center_x = (candidateArmor.keyPoint[4].x + candidateArmor.keyPoint[1].x + candidateArmor.keyPoint[2].x + candidateArmor.keyPoint[3].x) / 4;
        double center_y = (candidateArmor.keyPoint[4].y + candidateArmor.keyPoint[1].y + candidateArmor.keyPoint[2].y + candidateArmor.keyPoint[3].y) / 4;
        if(POINT_DIST(Point2f(center_x,center_y), candidateArmor.keyPoint[0]) > 5.5) continue;

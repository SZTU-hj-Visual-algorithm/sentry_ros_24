...
int main(int argc, char *argv[]){
    ...
    // 初始化节点
    ros::init(argc, argv, "R_image_publisher");
    ...
    image_transport::Publisher pub = it.advertise("/HIK_Camera_R/image", 1);
    ...
    if(HIK.HIKCamera_Init()){
        ...
        while (HIK.Camera_OK){
            ...
            // cv::imshow("src_R", HIK.src);

            // 填充数据
            cv_bridge::CvImage img_msg;
            img_msg.image = HIK.src.clone();
            img_msg.header.stamp = ros::Time::now();
            img_msg.header.frame_id = "HIK_Camera_R";
